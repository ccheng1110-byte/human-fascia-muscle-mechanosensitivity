...
# Step 17G: targeted S3 processed-matrix audit.
...
if (length(missing_inputs) > 0L) {
  stop("Missing Step 17G input(s): ", paste(missing_inputs, collapse = "; "))
}
...
safe_write_csv(summary, file.path(result_dir, "Step17G_S3_processed_matrix_audit_summary_v1.csv"))

decision_path <- file.path(result_dir, "Step17G_S3_processed_matrix_audit_decision_v1.md")
decision_lines <- c(
...
message("Step 17G S3 processed-matrix audit completed.")
for (i in seq_len(nrow(summary))) {
  message(
