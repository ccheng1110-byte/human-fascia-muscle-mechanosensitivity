...
# Step 17G3: corrected S3 processed-matrix audit with official-title mapping gate.
...
if (length(missing_inputs) > 0L) {
  stop("Missing Step 17G3 input(s): ", paste(missing_inputs, collapse = "; "))
}
...
audit_source <- function(gse_id) {
  ...
  mapping_complete <- nrow(eligible_mapping) == nrow(metadata) &&
    ...
    all(eligible_mapping$match_status %in% c(
      "matched_by_GSM", "matched_by_design_key", "matched_by_official_title"
    ))
...
safe_write_csv(summary, file.path(result_dir, "Step17G3_S3_processed_matrix_mapping_reaudit_summary_v1.csv"))

decision_path <- file.path(result_dir, "Step17G3_S3_processed_matrix_mapping_reaudit_decision_v1.md")
decision_lines <- c(
  "# Step 17G3 S3 corrected processed-matrix mapping re-audit",
  "",
...
message("Step 17G3 S3 corrected processed-matrix mapping re-audit completed.")
for (i in seq_len(nrow(summary))) {
  message(
